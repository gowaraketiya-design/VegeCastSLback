...
MODEL_DIR = os.path.join(BASE_DIR, "models", "red_onions_xgb")

MODEL_PATH = os.path.join(MODEL_DIR, "red_onions_xgboost_model.ubj")
META_PATH = os.path.join(MODEL_DIR, "red_onions_meta.json")
...
def main():
    try:
        ...
        if len(last_data) < 7:
            raise ValueError("At least 7 usable rows are required for Red Onions XGBoost forecast")
